...
dflist.remove[[n]] <- findCorrelation(cor(dplyr::select(dflist.num[[n]], -c(MosqPerL, WayPt_ID, LongX, LatY, Adj_X, Adj_Y, Day, Month, EpiWeek, INLAWeek))), cutoff = 0.5, names = TRUE)
}
...
bvs.list[[n]] <- GibbsBvs(MosqPerL ~ . , data = dplyr::select(dflist.reduced[[n]], -c(WayPt_ID, LongX, LatY, Adj_X, Adj_Y, Day, Month, EpiWeek, INLAWeek, Sampledtire, NSampledTires)), time.test=F)
...
dat.selected <- list()
for (n in 1:length(bvs.vars)) {
  dat.selected[[n]] <- cbind(dplyr::select(dflist[[n]], c(MosqPerL, WayPt_ID, LongX, LatY, Adj_X, Adj_Y, Day, Month, EpiWeek, INLAWeek, Sampledtire, NSampledTires)), dplyr::select(dflist[[n]], names(bvs.vars[[n]][1:5])))
}
